...
if snakemake.params.user_proteins:
    shell(
        "(/hpf/projects/CTrost/ajain/bacterial_genomics_pipeline/tools/bakta/bin/bakta --db {snakemake.params.db} --output {snakemake.params.outdir} --proteins {snakemake.params.user_proteins} --prefix {snakemake.params.prefix} {snakemake.input}/contigs.fa ) {log}"
    )
