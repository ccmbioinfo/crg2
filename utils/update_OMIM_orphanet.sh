...
#
# Usage:
#   sh update_OMIM_orphanet.sh <OMIM_key>
#
# Arguments:
...
cd cre
git checkout -b anno-update-${DATE}-hg19
...
git push origin anno-update-${DATE}-hg19
...
cd cre
git checkout hg38
git checkout -b anno-update-${DATE}-hg38 hg38
...
git commit -m "Update OMIM and orphanet"
git push origin anno-update-${DATE}-hg38
...
# update cre.vcf2db.R
previous_date=`grep 202 scripts/cre/cre.vcf2db.R | grep -v doi | rev | cut -d '_' -f1 | rev | tr -d '.tsv")'`
...
# update same files for CPHI-DRAGEN-anno
cd ../
git clone https://github.com/ccmbioinfo/CPHI-DRAGEN-anno
cd CPHI-DRAGEN-anno
git checkout -b anno-update-${DATE}

cp ../cre-hg19/data/OMIM_hgnc_join_omim_phenos_${DATE}.tsv  workflow/scripts/cre/data
cp ../cre-hg19/data/genemap2.txt ../cre-hg19/data/mim2gene.txt ../cre-hg19/data/mimTitles.txt ../cre-hg19/data/morbidmap.txt  workflow/scripts/cre/data
cp ../cre-hg19/data/orphanet.txt workflow/scripts/cre/data

# update cre.vcf2db.R
previous_date=`grep 202 workflow/scripts/cre/cre.vcf2db.R  | grep -v doi | rev | cut -d '_' -f1 | rev | tr -d '.tsv")'`
sed -i "s/${previous_date}/${DATE}/g" workflow/scripts/cre/cre.vcf2db.R

# push changes to crg2-pacbio repo
git add workflow/scripts/cre/data/OMIM_hgnc_join_omim_phenos_${DATE}.tsv workflow/scripts/cre/data/orphanet.txt workflow/scripts/cre/cre.vcf2db.R workflow/scripts/cre/data/genemap2.txt workflow/scripts/cre/data/mim2gene.txt workflow/scripts/cre/data/mimTitles.txt workflow/scripts/cre/data/morbidmap.txt
git commit -m "Update OMIM and orphanet"
git push origin anno-update-${DATE}
