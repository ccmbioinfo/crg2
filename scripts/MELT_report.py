...
hgmd_db = (
    "/hpf/largeprojects/ccm_dccforge/dccdipg/Common/annotation/HGMD_2018/hgmd_pro.db"
)
protein_coding_genes = "/hpf/largeprojects/ccm_dccforge/dccdipg/Common/annotation/grch37.p13.ensembl.sorted.protein.coding.genes.bed"
...
df = ann_records.calc_exons_spanned(df, exon_bed)

# annotate against gnomAD SV
df = annotate_gnomad(gnomad, df)

# annotate against hgmd
df = ann_records.annotate_hgmd(hgmd_db, df)

# add DECIPHER URL
ann_records.add_decipher_link(df)

# calculate distance to exon boundaries
df = ann_records.calc_exon_boundaries(df.reset_index(), exon_bed)
